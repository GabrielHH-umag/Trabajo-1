...
#include <stdarg.h>
//Funciones ugit
	void ugit_err(const char *format, ...) //Similar a printf(""), imprime "uGitError: " antes del string que se pase como argumento
	{
		va_list args;
	...
	void ugit_say(const char *format, ...)//Similar a ugit_err(""), imprime "uGit: " en vez de "uGiterror: "
	{
		va_list args;
		va_start(args, format);
		printf("uGit: ");
	...
	void init_repo(Rep_ *repo) //Inicializa los valores de la estructura Rep_ repo en 0 o Nulo
	{
		repo->nombre = NULL;
		repo->num_stage = 0;
		repo->commits = NULL;
...
//Funciones para hash
	int hash_file_sha1(const char *filename, char *output) //Funcion hash que utiliza git
	{
	...
	void generate_commit_id(const char *input, char *output) //Genera el id del commit 
	{
		unsigned char hash[SHA_DIGEST_LENGTH];
...
//Funciones que manipulan directorios y archivos
	...
	int ChangeDir(char *path) //Cambia el directorio
	{
		if (chdir(path) != 0)
		{
...
//Funciones para Cargar/Actualizar datos
	int LoadRepoData(Rep_ *repo) //Carga los datos del repositorio
	{
		if(DirExists("./.ugit") == 0)
		{
	...
	int UpdateRepoData(Rep_ *repo) //Actualiza los datos del repositorio
	{
	...
	int LoadCommitsData(Rep_ *repo) //Carga los datos del commit
	{
		if(DirExists("./.ugit") == 0)
		{
			ugit_err("No .ugit repository found\nTry: 'init <your_repo_name>' or 'init'\n");
...
//Funciones para obtener datos de usuario
	void get_user(char *buffer, size_t size) //Obtiene el nombre del usuario
	{
	...
	void get_date(char *buffer, size_t size) //Obtiene la fecha actual
	{
		time_t t = time(NULL);
